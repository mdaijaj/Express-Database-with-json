...
app.get("/create_table", function(req,res){      		//router or path endpoint
	let db=new sqlite3.Database("icc", function(err){   //open the database
	if(!err){
		db.run("create table if not  exists indian_cricket_team (id integer primary key autoincrement, name text, state text, city text)",function(exists){
		if(exists){
			console.log("table succesfully")
			return res.json("created table succesfully");
		}else{
...
app.listen(8080, function(){  //port which is running 
